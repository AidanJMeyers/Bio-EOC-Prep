...
import { exam, examConfig } from './data/exam.jsx';
...
export default function App() {
  ...
  const navigate = (target, anchor) => {
    setActive(target);
    if (anchor) {
      // delay so chapter renders before scrolling
      setTimeout(() => {
    ...
    <div className="min-h-screen flex bg-slate-50">
      ...
      <aside className="hidden md:flex flex-col w-72 bg-slate-900 text-slate-200 min-h-screen sticky top-0 max-h-screen overflow-y-auto scrollbar-thin">
        <div className="p-5 border-b border-slate-800">
          <div className="text-xs uppercase tracking-wider text-sky-400 font-bold">Study Dashboard</div>
          <div className="text-xl font-bold text-white mt-1">{examConfig?.subject || 'Course'}</div>
          <div className="text-xs text-slate-400 mt-1">Interactive review · saves progress</div>
        </div>
      ...
      <main className="flex-1 min-w-0 md:pt-0 pt-14">
        ...
        <footer className="max-w-5xl mx-auto px-4 py-10 text-center text-xs text-slate-400">
          Interactive study dashboard · progress persists via window.storage
        </footer>
      </main>
